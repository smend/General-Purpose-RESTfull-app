...
public class GreetingController extends BaseController {
    ...
    public ResponseEntity<Collection<GreetingDTO>> getGreetings() {
        ...
        Collection<GreetingDTO> greetingsDTO = greetingService.ToDTOs(greetingService.findAll());

        logger.info("< getGreetings");
        return new ResponseEntity<Collection<GreetingDTO>>(greetingsDTO,
                HttpStatus.OK);
    }
    ...
    @RequestMapping(
            ...
            method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<GreetingDTO> getGreeting(@PathVariable("id") Long id) {
        logger.info("> getGreeting id:{}", id);
         
        
        
        
        GreetingDTO GreetingDTO = greetingService.greetingToDTO(greetingService.findOne(id));
        
       

        logger.info("< getGreeting id:{}", id);
        return new ResponseEntity<GreetingDTO>(GreetingDTO, HttpStatus.OK);
    }
    ...
    public ResponseEntity<GreetingDTO> createGreeting(
        ...
        GreetingDTO dto = greetingService.greetingToDTO(savedGreeting);

        logger.info("< createGreeting");
        return new ResponseEntity<GreetingDTO>(dto, HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<GreetingDTO> updateGreeting(
        ...
        if (updatedGreeting == null) {
            return new ResponseEntity<GreetingDTO>(
                    HttpStatus.INTERNAL_SERVER_ERROR);
        }
        
        GreetingDTO dto = greetingService.greetingToDTO(updatedGreeting);

        logger.info("< updateGreeting id:{}", greeting.getId());
        return new ResponseEntity<GreetingDTO>(dto, HttpStatus.OK);
    }
...
//        return null; //Greeting("Hello, " + ((Logger) message).getName() + "!");
//    }
